...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        while start < len(text):
            ...
            # If we're not at the end, try to break at sentence boundaries
            if end < len(text):
                ...
                for ending in sentence_endings:
                    ...
                    if last_ending != -1 and last_ending > best_break - 200:  # Don't go too far back
            ...
            if chunk:  # Only add non-empty chunks
                ...
                chunk_metadata['total_chunks'] = None  # Will be set later
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        if update_mode == 'replace':
            # Delete all existing documents from this file
            file_docs = self.get_file_documents(file_path)
            if file_docs:
                self.collection.delete(ids=file_docs)
                logger.info(f"Deleted {len(file_docs)} existing documents from {file_path}")
